...
public class OrderDetailImpl implements OrderDetailService {
    ...
    public Map<Object, Object> save(OrderDetail request) {
        try {
            ...
            Integer stock = checkDataDBProduct.get().getQuantity();
            if (request.getQuantity() == 0) return templateResponse.error("minimum order is 1");
            if (stock == 0) return templateResponse.error("out of stock");
            Integer updateStock = stock - request.getQuantity();
            checkDataDBProduct.get().setQuantity(updateStock);
            productRepository.save(checkDataDBProduct.get());
            if (updateStock < 0) throw new TransactionSystemException("Exceed available stock, current available stock is " + stock);
